...
static unsigned long disk_mb = 1;
module_param(disk_mb, ulong, 0644);
...
static int my_block_open(struct gendisk *gd, blk_mode_t mode)
{
	return 0;
}
...
static void my_xfer_request(struct my_block_dev *dev, struct request *req)
{
	...
	rq_for_each_segment(bvec, req, iter) {
		...
		if (verbose)
			printk(KERN_LOG_LEVEL "%s: buf %8p offset %lu len %zu dir %d\n", __func__, buffer, offset, len, dir);
...
static int create_block_device(struct my_block_dev *dev)
{
	...
	dev->size = disk_mb * 1024UL * 1024UL;
	dev->size = (dev->size / KERNEL_SECTOR_SIZE) * KERNEL_SECTOR_SIZE;
	if (dev->size == 0) {
		err = -EINVAL;
		goto out_vmalloc;
	}
	...
	lim.logical_block_size = KERNEL_SECTOR_SIZE;
	lim.physical_block_size = KERNEL_SECTOR_SIZE;
	lim.io_min = KERNEL_SECTOR_SIZE;
	...
	snprintf(dev->gd->disk_name, sizeof(dev->gd->disk_name), "myblock");
